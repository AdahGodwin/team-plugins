...
const DailyLogCard = () => {
    ...
    if (submitted) {
        return (
            <div className="rounded-2xl border border-teal-200 bg-teal-50 p-8 flex flex-col items-center text-center gap-4 shadow-sm h-full justify-center">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm ring-1 ring-slate-50 flex flex-col">
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-slate-900">Today's Check-in</h2>
                ...
                {step === 1 ? (
                        ...
                        <div>
                            ...
                            <div className="flex gap-3 items-center">
                                <input
                                    ...
                                    className="flex-1 text-base border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-300 bg-slate-50 placeholder:text-slate-300"
                                />
                    ...
                    <div className="space-y-5">
                                ...
                                {SYMPTOMS.map(s => (
                                    ...
                                    >
                                        <span className={`w-4 h-4 rounded-md border flex items-center justify-center shrink-0 transition-all ${
                                            symptoms.includes(s) ? 'bg-rose-500 border-rose-500' : 'border-slate-300'
                                        }`}>
                        ...
                        <div className="flex gap-3">
                            <button
                                onClick={() => setStep(1)}
                                className="flex-1 bg-slate-50 border border-slate-200 text-slate-600 font-semibold py-3.5 rounded-2xl hover:bg-slate-100 transition-all"
                            >
                                ← Back
                            </button>
                            <button
                                ...
                                disabled={loading}
                                className="flex-1 bg-linear-to-r from-blue-500 to-teal-500 text-white font-bold py-3.5 rounded-2xl hover:opacity-95 transition-all shadow-md disabled:opacity-40 active:scale-[0.98] flex items-center justify-center gap-2"
                            >
                                {loading
